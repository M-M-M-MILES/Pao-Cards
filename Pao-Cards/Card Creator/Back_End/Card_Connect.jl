...
function createDeck(deckName, userId)
    try
        SQLite.execute(db, "INSERT INTO Decks VALUES ('$deckName')")
        catch
            println("Error creating Deck")
    end

    deckId = SQLite(db, "SELECT deckID WHERE deckName = '$deckName'")

    try
        SQLite.execute(db, "INSERT INTO User_Decks VALUES ('$userId', '$deckId')")
        catch
...
function addToDeck(deckId, cardId)
    try
        SQLite.execute(db, "INSERT INTO Deck_Cards VALUES ('$deckId')")
        catch
